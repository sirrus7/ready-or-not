...
interface ChoicePanelProps {
    ...
    currentSlide: Slide; // REFACTOR: Changed from currentPhase
    ...
    return (
        <div className="space-y-3">
            {challengeOptions.map((opt) => (
                <label key={opt.id} className={`flex items-start p-4 rounded-lg transition-all cursor-pointer border-2
                    ${selectedChallengeOptionId === opt.id ? 'bg-blue-600/80 border-blue-400 text-white shadow-md' : 'bg-gray-600/70 border-gray-500 hover:bg-gray-500/70'}`}>
                    <input type="radio" name={`challengeOption-${currentSlide.id}`}
                           ...
                           checked={selectedChallengeOptionId === opt.id} onChange={() => onChallengeSelect(opt.id)}
                           disabled={isSubmitting}/>
                    <div className="ml-4 text-sm flex-grow">
                        <div className="leading-relaxed"><span className="font-semibold">{opt.id}. </span>{opt.text}
                        </div>
                        {opt.estimated_cost !== undefined && (<div
                            className="text-xs opacity-80 mt-2 bg-gray-700/50 rounded px-2 py-1 inline-block">Cost/Savings: {formatCurrency(opt.estimated_cost)}</div>)}
